...
function leftOrBoth(defaultLeft) {
    return function (ma) { return (Option_1.isNone(ma) ? left(defaultLeft) : both(defaultLeft, ma.value)); };
}
...
function rightOrBoth(defaultRight) {
    return function (me) { return (Option_1.isNone(me) ? right(defaultRight) : both(me.value, defaultRight)); };
}
...
exports.these = {
    ...
    bimap: function (fla, f, g) {
        return isLeft(fla) ? left(f(fla.left)) : isRight(fla) ? right(g(fla.right)) : both(f(fla.left), g(fla.right));
    },
    mapLeft: function (fla, f) { return exports.these.bimap(fla, f, identity); },
