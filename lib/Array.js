 ...
 * @since 2.0.0
 */
function isOutOfBound(i) {
    return function (as) { return i < 0 || i >= as.length; };
}
 ...
 */
function lookup(i) {
    var is = isOutOfBound(i);
    return function (as) { return (is(as) ? Option_1.none : Option_1.some(as[i])); };
}
...
function updateAt(i, a) {
    var is = isOutOfBound(i);
    return function (as) { return (is(as) ? Option_1.none : Option_1.some(unsafeUpdateAt(i, a, as))); };
}
...
function deleteAt(i) {
    var is = isOutOfBound(i);
    return function (as) { return (is(as) ? Option_1.none : Option_1.some(unsafeDeleteAt(i, as))); };
}
...
function modifyAt(i, f) {
    var is = isOutOfBound(i);
    return function (as) { return (is(as) ? Option_1.none : Option_1.some(unsafeUpdateAt(i, f(as[i]), as))); };
}
 ...
 */
function elem(E) {
    return function (a) { return function (as) {
 ...
 */
function chunksOf(n) {
    var is = isOutOfBound(n - 1);
    return function (as) { return (is(as) ? [as] : chop(splitAt(n))(as)); };
}
